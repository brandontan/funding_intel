...
- Set up the São Paulo VPS proxy (Node + Squid) with systemd + ufw for `http://216.238.116.7:4000`, secured by `x-proxy-key`, so Binance/Bybit fetchers can route traffic through it.
...
2. Update fetcher envs (`BINANCE_PROXY_URL`, `BYBIT_PROXY_URL`, `*_KEY`) and run `npm run ingest` to backfill Binance/Bybit data through the new VPS proxy.
...
- None—VPS proxy is live at `216.238.116.7:4000` (set `x-proxy-key` envs as documented).
